...
IF EXIST "%REPODIR%\data\CIRVIE_INCIDENTS_2026.xlsm" (
    echo [OK] CIRVIE_INCIDENTS_2026.xlsm existe deja. Rien a faire.
    echo      Pour recreer le fichier, supprimez data\CIRVIE_INCIDENTS_2026.xlsm d'abord.
    pause
    exit /b 0
)

IF NOT EXIST "%REPODIR%\data\CIRVIE_INCIDENTS_2026.xlsx" (
    echo [ERREUR] data\CIRVIE_INCIDENTS_2026.xlsx introuvable.
    echo          Faites un "git pull" pour recuperer le fichier.
    pause
    exit /b 1
)

IF NOT EXIST "%REPODIR%\predict_server.exe" (
    echo [ATTENTION] predict_server.exe manquant.
    echo             Telechargez-le depuis les releases GitHub ou executez scripts\build_exe.bat.
    echo             Vous pourrez relancer ce script apres.
    pause
)

echo Conversion de CIRVIE_INCIDENTS_2026.xlsx en .xlsm avec macros...
powershell -ExecutionPolicy Bypass -File "%~dp0setup_excel.ps1" "%REPODIR%"
